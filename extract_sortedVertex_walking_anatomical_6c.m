...
load('rigidBody_anatomical_markers_info_data.mat');
...
for j=activity_start_idx:activity_end_idx
    ...
    current_technical_frame_markers_info = technical_frame_markers_rigidBody_info.(current_activity_name);
    current_rigidBody_anatomical_markers_info = rigidBody_anatomical_markers_info.(current_activity_name);
    
    input_data_val.activity_idx_val = current_activity_idx;
    input_data_val.technical_frame_markers_info_val = current_technical_frame_markers_info;
    input_data_val.rigidBody_anatomical_markers_info_val = current_rigidBody_anatomical_markers_info;
...
function [local_vectors_output, marker_output] = generate_solidification_data(input_data, activity)
    ...
    rigidBody_anatomical_markers_info = input_data.rigidBody_anatomical_markers_info_val;
    ...
    for segment_idx=1:length(segment_list)
        ...
        for side_idx=1:length(side_list)
            ...
            segment_meta_data_struct.rigidBody_anatomical_markers_info_val = rigidBody_anatomical_markers_info;
...
function [local_vectors_output, marker_output] = triangle_solidification(segment_meta_data, activity)
    ...
    rigidBody_anatomical_markers_info = segment_meta_data.rigidBody_anatomical_markers_info_val;
    rigidBody_markers_struct = rigidBody_anatomical_markers_info(subject_idx).(segment).(side);
    ...
    for j=1:3
        ...
        if isfield(activity(activity_idx).VICON(subject_idx, 1).movement_data, marker_x)
            marker_info(j).x = rigidBody_markers_struct(j).x;
            marker_info(j).y = rigidBody_markers_struct(j).y;
            marker_info(j).z = rigidBody_markers_struct(j).z;
        else
