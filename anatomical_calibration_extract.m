...
anatomical_calibration_parameters_info = struct;
            ...
            for m=1:num_marker
                ...
                if ~static_cal_empty
                    ...
                    if current_technical_markers_present && current_anatomical_markers_present
                        ...
                        current_anatomical_marker_info(m).x = mean(position_local_x_list);
                        current_anatomical_marker_info(m).y = mean(position_local_y_list);
                        current_anatomical_marker_info(m).z = mean(position_local_z_list);
                    end
                end
            end
            anatomical_calibration_parameters_info(current_subject_idx).(current_segment_name).(current_side) = current_anatomical_marker_info;
            disp('----finished processing this round----')
        end
    end
end
save('anatomical_calibration_parameters_info_data', 'anatomical_calibration_parameters_info', '-v7.3');
