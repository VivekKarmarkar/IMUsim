...
ground_contact_visualization_flag = true;
            ...
            for s=1:num_segments
                ...
                for n=1:row_subplot
                    for m=1:col_subplot
                        ...
                        x_qt.processed = current_vector_processed.x;
                        y_qt.processed = current_vector_processed.y;
                        z_qt.processed = current_vector_processed.z;
            ...
            if ~all(isnan(time_vector_val))
                ...
                for j=1:num_subplot
                    ...
                    if(~isempty(right_qt.raw) && ~isempty(left_qt.raw))
                        ...
                        title(title_subplot);
                        if j==1
                            description_axes = zeros(8, 1);
                            description_axes(1) = plot(NaN,NaN,'--r');
                            description_axes(2) = plot(NaN,NaN,'r');
                            description_axes(3) = plot(NaN,NaN,'--b');
                            description_axes(4) = plot(NaN,NaN,'b');
                            description_axes(5) = plot(NaN,NaN,'-m');
                            description_axes(6) = plot(NaN,NaN,'--m');
                            description_axes(7) = plot(NaN,NaN,'-g');
                            description_axes(8) = plot(NaN,NaN,'--g');
                            legend(description_axes, 'RightLegRaw: RedDash', 'RightLegProcessed: Red', 'LeftLegRaw: BlueDash', 'LeftLegProcessed: Blue', 'RightHS: Magenta', 'RightTO: MagentaDash', 'LeftHS: Green', 'LeftTO: GreenDash');
                        end
                    end
                end
            end
