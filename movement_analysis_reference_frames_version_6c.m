...
now_rigidBody_anatomical_markers_info = rigidBody_anatomical_markers_info.(activity_name);
...
input_data_val.rigidBody_anatomical_markers_info_val = now_rigidBody_anatomical_markers_info;
...
function [local_vectors_output, marker_output] = triangle_solidification(segment_meta_data, activity)
    ...
    technical_markers_list = [rigidBody_markers_struct(1).name; rigidBody_markers_struct(2).name; rigidBody_markers_struct(3).name];
    ...
    marker_info = struct;
    for j=1:3
        if ~isempty(rigidBody_markers_struct(j).name)
