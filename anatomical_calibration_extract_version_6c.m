...
load_uvm_data_processed_bool = false;
...
%% GENERATE AND POPULATE DATA CONTAINERS WHILE ACCOUNTING FOR SET PREFERENCES
...
for j=activity_start_idx:activity_end_idx
    ...
    current_activity_name = strrep(activity(current_activity_idx).name, " ", "_");
    technical_frame_markers_info = technical_frame_markers_rigidBody_info.(current_activity_name);
    input_data_val.technical_frame_markers_info_val = technical_frame_markers_info;
    input_data_val.sorted_vertex_old_info_val = sorted_vertex_walking_rigidBody_info.(current_activity_name);
    ...
    for n=1:num_subjects
        ...
        for seg_idx=1:num_body_segment
            current_segment = body_segment_input(seg_idx).name;
            for k=1:num_side
                ...
                anatomical_calibration_parameters_processed_rigid_body_info.(current_activity_name)(current_subject_idx).(current_segment).(current_side) = current_anatomical_marker_info;
